...
class TrainerWithGeneration(transformers.Seq2SeqTrainer):
    ...
    def evaluation_loop(
        ...
        # Evaluate actions
        average_error_vx, average_error_vy, average_error_vz = eval_velocity_errors(all_pred, all_label)
        if average_error_vx is not None and average_error_vy is not None and average_error_vz is not None:
            print(f"Average velocity error: vx: {average_error_vx}, vy: {average_error_vy}, vz: {average_error_vz}")
            wandb.log({"velocity_error_vx": average_error_vx})
            wandb.log({"velocity_error_vy": average_error_vy})
            wandb.log({"velocity_error_vz": average_error_vz})
        return eval_output
