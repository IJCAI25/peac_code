...
INSTRUCTION = "You are a certified professional evacuation drill instructor and please tell me step by step how to walk or run based on the input scenario."
ACTION_Q = [
    "How are you going to walk in this situation?",
    "What actions are you taking?",
    "Why are you taking these specific walking actions?",
    "How are you navigating this situation?",
    "What actions will be taken in this situation?",
    "How are you walking in this situation?",
...
def get_velocity_vx(text):
    match = re.search(r"vx (-?\d+\.?\d*)m/s", text)
    return float(match.group(1)) if match else None

def get_velocity_vy(text):
    match = re.search(r"vy (-?\d+\.?\d*)m/s", text)
    return float(match.group(1)) if match else None

def get_velocity_vz(text):
    match = re.search(r"vz (-?\d+\.?\d*)m/s", text)
    return float(match.group(1)) if match else None

def eval_velocity_errors(all_pred, all_label):
    accumulated_error_vx = 0.0
    accumulated_error_vy = 0.0
    accumulated_error_vz = 0.0
    total_number = 0

    for pred, label in zip(all_pred, all_label):
        pred_vx = get_velocity_vx(pred)
        label_vx = get_velocity_vx(label)
        pred_vy = get_velocity_vy(pred)
        label_vy = get_velocity_vy(label)
        pred_vz = get_velocity_vz(pred)
        label_vz = get_velocity_vz(label)

        if None in (pred_vx, label_vx, pred_vy, label_vy, pred_vz, label_vz):
            continue

        error_vx = abs(pred_vx - label_vx)
        error_vy = abs(pred_vy - label_vy)
        error_vz = abs(pred_vz - label_vz)

        accumulated_error_vx += error_vx
        accumulated_error_vy += error_vy
        accumulated_error_vz += error_vz
        total_number += 1

    average_error_vx = accumulated_error_vx / total_number if total_number > 0 else 0
    average_error_vy = accumulated_error_vy / total_number if total_number > 0 else 0
    average_error_vz = accumulated_error_vz / total_number if total_number > 0 else 0

    return average_error_vx, average_error_vy, average_error_vz


def get_control_longitudinal(text):
    match = re.search(r"- vx ([+-]?\d*\.\d+|\d+)m/s", text)
    if match:
        vx_value = float(match.group(1))
        # 假设 vx 的范围为 [-1, 1]，其中正值表示加速，负值表示刹车
        # 根据您的实际范围调整这一部分
        control_longitudinal = (vx_value + 2.0) / 4.0
        return control_longitudinal
    return None  # 如果没有找到 vx，返回 None

def get_control_lateral(text):
    match = re.search(r"- vy ([+-]?\d*\.\d+|\d+)m/s", text)
    if match:
        vy_value = float(match.group(1))
        # vy 直接作为横向控制值，正值可能表示向左，负值表示向右
        # 根据您的坐标系统调整这一解释
        return vy_value
    return None  # 如果没有找到 vy，返回 None
